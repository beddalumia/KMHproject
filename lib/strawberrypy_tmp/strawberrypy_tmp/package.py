...
class FiniteModel:
    """
    A class describing a finite model with either TBmodels or PythTB instances. Key features are the ability to add disorder,
    vacancies to the models, calculate local topological markers and localization markers.
    ...
    def __init__(self, tbmodel = None, nx_sites : int = 1, ny_sites : int = 1, spinful : bool = False, mode : str = 'tb', fnames = None, uc_vol : float = 1, atoms_uc : int = 1):
        ...
        if spinful:
            if mode == 'tb':
                ...
                if len(fnames) == 0 or len(fnames) == 4:
                    self._sz = self._load_spin("szspin.input" if fnames == None else fnames[3])
                elif len(fnames) == 3:
                    self._sz = self._load_spin("szspin.input" if fnames == None else fnames[2])
                else:
                    raise RuntimeError("Invalid number of file names passed to the constructor of FiniteModel")
        else:
            self._sz = None
        ...
        if mode == 'tb':
            ...
            if len(fnames) == 0 or len(fnames) == 4:
                positions = self._load_positions(["xcoordinates.input" if fnames == None else fnames[1], "ycoordinates.input" if fnames == None else fnames[2]])
            elif len(fnames) == 3:
                positions = self._load_positions(fnames[1])
            else:
                raise RuntimeError("Invalid number of file names passed to the constructor of FiniteModel")
        ...
        if mode == 'tb':
            ...
            self._hamiltonian, self._nocc = self._load_hamiltonian("hamiltonian.input" if fnames == None else fnames[0])
    ...
    def _load_positions(self, filename):
        ...
        if len(filename) == 2:
            ...
            latpos = np.loadtxt(filename, unpack = True)
            rx = latpos[0]; ry = latpos[1]
            
            if self._spinful:
                rx = np.array([latpos[0], latpos[0]]).flatten()
                ry = np.array([latpos[1], latpos[1]]).flatten()
    ...
    def _load_hamiltonian(self, filename):
        ...
        """
        F = np.loadtxt(filename, dtype = np.complex128)
        N = min(F.shape)
        H = F[:N, :]
        H += 1.j * F[N:, :]
        return H, int(len(H) / 2)
    ...
    def _initialize_mask(self):
        ...
        if isinstance(self._model, tbm.Model):
            ...
            return np.array([True for _ in range(len(self._hamiltonian))])
        else:
            raise NotImplementedError("Invalid model instance.")
    ...
    def add_vacancies(self, vacancies_list):
        ...
        Args:
            - vacancies_list: a list of [cell_x, cell_y, basis] pointing to the cell and atom to be removed. Multiple values at once are alloed
        """
        ...
        if np.array(vacancies).shape == ():
            ...
            if not self._spinful:
                ...
                newx = np.delete(pos[0], vacancies)
                newy = np.delete(pos[1], vacancies)
                if sz is not None:
                ...
                for _ in range(2):
                    ham = np.delete(ham, vacancies, 0)
                    ham = np.delete(ham, vacancies, 1)
                    newx = np.delete(newx, vacancies)
                    newy = np.delete(newy, vacancies)
                    if sz is not None:
                        sz = np.delete(sz, vacancies, 0)
                        sz = np.delete(sz, vacancies, 1)
    ...
    def local_spin_chern_marker(self, direction : int = None, start : int = 0, return_projector : bool = None, projector : np.ndarray = None, macroscopic_average : bool = False, cutoff : float = 0.8, check_gap : bool = False, histogram : bool = False):
        """
        Evaluate the spin Chern marker on the whole lattice if direction is None. If direction is not None evaluates the spin Chern marker along direction starting from start.
            ...
            - histogram : if True, returns the histogram of the eigenvalues of PSzP as a file "pszp_histogram.pdf" (default is False)
        ...
        if projector is None:
            ...
            if check_gap and evals[int(self._nocc // 2)] - evals[int(self._nocc // 2) - 1] < 1e-14:
                raise RuntimeError("PSzP gap closes!")
            
            # If histogram, make an histogram of the eigenvalues of PSzP
            if histogram:
                fig, ax = plt.subplots(1, 1)
                ax.set_xlabel(r"$\lambda_{PS_zP}$")
                ax.set_ylabel("#")
                ax.set_title(r"Histogram of the eigenvalues of $PS_zP$")
                counts, bins = np.histogram(evals, bins = int(0.5 * len(evals)))
