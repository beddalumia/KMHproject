...
%for iSOI = 1:Nlines
    SOIDIR = "SOI=0.060000";%SOI_names(iSOI);
    ...
    save('soectral_line.mat','DOS','SE','U_list');
    cd('..');
%end
...
function [DOS,SE,U_list] = spectral_line(U_LIST)
%% Getting a list of spectral functions, from directories.
%  U_LIST: an array of values for Hubbard interaction U (could be empty!)
%  DOS: a cell of tensors
%  SE:  a cell of tensors
%  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    global ignUlist
    if isempty(U_LIST) | ignUlist == true
       [U_LIST, ~] = get_list('U'); 
    ...
    for iU = 1:length(U_LIST)
        ...
        cd(UDIR); 
        [cellDOS{iU}, cellSE{iU}] = get_spectrals();
        cd('..');
    end
    U_list = U_LIST;
end

function [Ew, Gw] = get_spectrals()
%% 
%  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    Gw = importdata(append(TitleString,'\Greal_l11_s1_realw_indx000001.dat'));
    %%% Structure %%%%%%%%%%%%%%%%%%
