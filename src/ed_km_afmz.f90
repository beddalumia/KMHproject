program ed_kanemele
   ...
   do while(.not.converged.AND.iloop<nloop)
      ...
      if(neelsym)then
         ...
         Smats(2,2,2,:,:,:) = Smats(1,1,1,:,:,:) !Σ(iω)_{B,dw,dw} = Σ(iω)_{A,up,up}
         Smats(2,1,1,:,:,:) = Smats(1,2,2,:,:,:) !Σ(iω)_{B,up,up} = Σ(iω)_{A,dw,dw}
         Sreal(2,2,2,:,:,:) = Sreal(1,1,1,:,:,:) !Σ(ω)_{B,dw,dw}  = Σ(ω)_{A,up,up}
         Sreal(2,1,1,:,:,:) = Sreal(1,2,2,:,:,:) !Σ(ω)_{B,up,up}  = Σ(ω)_{A,dw,dw}
         if(master)write(*,*) "************************************"
         if(master)write(*,*) "*                                  *"
